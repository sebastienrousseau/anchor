...
// NotInstalledError reports a message Anchor knows about but has no schema for.
...
func (e *NotInstalledError) Error() string {
	...
	if !e.Known {
		...
		b.WriteString("Anchor knows every message identifier in the published standard,\n")
		b.WriteString("so this is probably a typo. Try: anchor search " + firstToken(e.Query) + "\n")
		return b.String()
	}
	...
	b.WriteString("\nThen import it:\n\n")
	b.WriteString("  anchor catalog add ~/Downloads/<downloaded>.zip\n")
	return b.String()
}
...
// notInstalled builds the guidance for a message Anchor cannot open.
...
func lightModeNotice(what string) string {
	return fmt.Sprintf(
		"\n%s %s from Anchor's embedded index of the standard.\n"+
			"      Install schemas for full detail: anchor catalog add <zip>  (see: anchor catalog where)\n",
