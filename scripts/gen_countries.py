# ISO 3166-1 alpha-2 officially assigned codes with their short names.
DATA = """
...
MA Morocco|MC Monaco|MD Moldova|ME Montenegro|MF Saint Martin (French part)
...
PS Palestine, State of|PT Portugal|PW Palau|PY Paraguay|QA Qatar|RE Réunion
...
TO Tonga|TR Türkiye|TT Trinidad and Tobago|TV Tuvalu|TW Taiwan
...
for chunk in DATA.replace("\n", "|").split("|"):
    ...
    entries.append((code, name.strip()))

entries.sort()
seen = set()
for c, _ in entries:
    assert len(c) == 2 and c.isupper(), c
    assert c not in seen, "duplicate " + c
    seen.add(c)

out = ['// SPDX-FileCopyrightText: 2026 Sebastien Rousseau <sebastian.rousseau@gmail.com>',
...
for c, n in entries:
    out.append('\t%-6s %s,' % ('"%s":' % c, '"%s"' % n.replace('"', '\\"')))
out.append('}')
out.append('')
out.append('''// IsCountryCode reports whether s is an assigned ISO 3166-1 alpha-2 code.
...
open('internal/rules/countries.go', 'w').write("\n".join(out) + "\n")
print("codes:", len(entries))
