...
// Most ISO 20022 code sets are enumerated in the schemas, and Anchor reads them
...
//
// Anchor does not redistribute that publication any more than it redistributes
...
func ImportExternalSets(path string) (*ExternalSets, error) {
	...
	if len(sets.Codes) == 0 {
		return nil, fmt.Errorf("%s holds no external code sets Anchor recognises",
			filepath.Base(path))
	}
	sets.Source = path
	sets.index()
...
func importExternalJSON(path string) (*ExternalSets, error) {
	...
	if err := json.Unmarshal(data, &grouped); err != nil {
		return nil, fmt.Errorf("%s is not a shape Anchor recognises: %w", filepath.Base(path), err)
	}
