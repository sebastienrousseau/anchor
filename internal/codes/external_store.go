...
// Anchor can read quickly. Tab-separated text is enough: a few thousand rows,
...
// the user's copy of a publication Anchor does not redistribute.

// externalCodesFile is where an imported publication is stored, relative to the
// catalogue root.
const externalCodesFile = ".anchor/external-codes.tsv"
...
func SaveExternalSets(root string, sets *ExternalSets) (string, error) {
	...
	w := bufio.NewWriter(f)
	_, _ = fmt.Fprintf(w, "# ISO 20022 external code sets, imported by Anchor from %s\n",
